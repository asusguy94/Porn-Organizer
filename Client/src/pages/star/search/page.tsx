...
function Filter() {
  const [websites, setWebsite] = useState<General[]>([])
  const [starData, setStarData] = useState<StarInfo>({ breast: [], ethnicity: [], haircolor: [] })

  useEffect(() => {
    axios.get<StarInfo>(`${serverConfig.api}/star/info`).then(({ data }) => setStarData(data))
    axios.get<General[]>(`${serverConfig.api}/website`).then(({ data }) => setWebsite(data))
  }, [])
