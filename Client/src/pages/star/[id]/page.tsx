...
import { SetState, Similar, StarVideo } from '@/types'
...
export default function StarPage() {
  ...
  useEffect(() => {
    axios.get<{ star: Star; videos: StarVideo[] }>(`${serverConfig.api}/star/${id}`).then(({ data }) => {
      setStar(data.star)
    ...
    <Card>
      ...
      <CardContent>
        <Grid id={styles.similar}>
          {similar.map(star => (
            <Link key={star.id} to={`/star/${star.id}`} className={styles.star}>
      ...
      {star.image !== null ? (
        <>
          <ContextMenuTrigger id='star__image'>
            <Image id={styles.profile} src={`${serverConfig.api}/star/${star.id}/image`} width={200} height={275} />
          </ContextMenuTrigger>
...
function StarForm({ star, update }: StarFormProps) {
  const [starInfo, setStarInfo] = useState<StarInfo>({ breast: [], haircolor: [], ethnicity: [] })
  useEffect(() => {
    axios.get<StarInfo>(`${serverConfig.api}/star/info`).then(({ data }) => setStarInfo(data))
  }, [])
  ...
  return (
    <>
      ...
      <StarInputForm update={updateInfo} name='Breast' value={star.info.breast} list={starInfo.breast} capitalize />

      <StarInputForm
        update={addHaircolor}
        name='Haircolor'
        value={star.info.haircolor}
        list={starInfo.haircolor}
        emptyByDefault
      >
        <InputFormData label='haircolor' data={star.info.haircolor} remove={removeHaircolor} />
      </StarInputForm>

      <StarInputForm update={updateInfo} name='Ethnicity' value={star.info.ethnicity} list={starInfo.ethnicity} />
