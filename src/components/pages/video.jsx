...
class VideoPage extends Component {
    ...
    handleWheel(e) {
        this.player.player.currentTime +=
            this.state.seekSpeed.wheel * Math.sign(e.deltaY) * -1
    }
    ...
    handleBookmark_clear() {
        Axios.get(
            `${config.api}/removebookmarks.php?id=${this.state.video.id}`
        ).then(() => {
            this.setState(() => {
    ...
    handleBookmark_time(id) {
        ...
        Axios.get(
            `${config.api}/changebookmarktime.php?id=${id}&time=${time}`
        ).then(({ data }) => {
            if (data.success) {
    ...
    handlePlays_reset() {
        Axios.get(
            `${config.api}/removeplays.php?videoID=${this.state.video.id}`
        ).then(() => {
            this.setState((prevState) => {
    ...
    handleDate_fix() {
        Axios.get(
            `${config.api}/fixvideodate.php?id=${this.state.video.id}`
        ).then(({ data }) => {
            if (data.success) {
                this.setState((prevState) => {
                    let video = prevState.video
    ...
    handleAttribute_add(attribute) {
        Axios.get(
            `${config.api}/addattribute.php?videoID=${this.state.video.id}&attributeID=${attribute.id}`
        ).then(({ data }) => {
            if (data.success) {
                this.setState((prevState) => {
    ...
    handleLocation_add(location) {
        Axios.get(
            `${config.api}/addlocation.php?videoID=${this.state.video.id}&locationID=${location.id}`
        ).then(({ data }) => {
            if (data.success) {
                this.setState((prevState) => {
                ...
                <section className='col-10'>
                        ...
                        <div className='col-12'>
                            <h1 className='header__title h2 align-middle'>
                                ...
                                <ContextMenu id='title'>
                                    ...
                                    <MenuItem
                                        onClick={() => this.handleTitle_copy()}
                                    >
                                        <i className='far fa-copy' /> Copy Title
                                    </MenuItem>

                                    <MenuItem
                                        onClick={() => this.handleStar_copy()}
                                    >
                            ...
                            <div className='header__locations'>
                                {this.state.loaded.video &&
                                    Object.keys(this.state.video.locations).map(
                                        (i) => (
                                            <div
                                                key={i}
                                                className='btn btn-sm btn-outline-danger location'
                                            >
                                                <ContextMenuTrigger
                                                    id={`location-${i}`}
                                                >
                            ...
                            <div className='header__attributes'>
                                {this.state.loaded.video &&
                                    Object.keys(
                                        this.state.video.attributes
                                    ).map((i) => (
                                        <div
                                            key={i}
                                            className='btn btn-sm btn-outline-primary attribute'
                                        >
                                            <ContextMenuTrigger
                                                id={`attribute-${i}`}
                                            >
                    ...
                    >
                        ...
                        <ContextMenu id='video'>
                            ...
                            <MenuItem
                                onClick={() => this.handleBookmark_clear()}
                            >
                            ...
                            Object.keys(this.state.bookmarks).map((i) => (
                                <React.Fragment key={i}>
                                    <div
                                        ...
                                        onClick={() =>
                                            this.handleBookmark_play(
                                                this.state.bookmarks[i].start
                                            )
                                        }
                                        ref={(bookmark) =>
                                    ...
                                    <ContextMenu id={`bookmark-${i}`}>
                                        ...
                                        >
                                            <i className='far fa-edit' /> Change
                                            Category
                                        </MenuItem>

                                        <MenuItem
                                            onClick={() =>
                                                this.handleBookmark_time(
                            ...
                            <React.Fragment>
                                <div className='star'>
                                    <ContextMenuTrigger id='star'>
                                        ...
                                        />
                                        <a
                                            href={`${config.source}/star.php?id=${this.state.star.id}`}
    ...
    componentDidUpdate(prevProps, prevState, snapshot) {
        if (this.state.loaded.video) {
            ...
            if (!this.state.loaded.videoEvents) {
                this.player.player.on('timeupdate', () => {
                    if (this.player.player.currentTime) {
                        localStorage.bookmark = Math.round(
                            this.player.player.currentTime
                        )
                    }
                })
            ...
            if (!this.state.loaded.hls && Hls.isSupported()) {
                ...
                hls.loadSource(
                    this.player.player.media.firstElementChild.getAttribute(
                        'src'
                    )
                )
                ...
                hls.on(Hls.Events.MANIFEST_PARSED, (e, data) => {
                    ...
                    if (desiredStartLevel > maxStartLevel)
                        desiredStartLevel = maxStartLevel
                    // Check if start level is too high
                    if (desiredStartLevel > dataLevels)
                        desiredStartLevel = dataLevels - 1
                    ...
                    if (Number(localStorage.video) === this.state.video.id) {
                        ...
                        if (Boolean(Number(localStorage.playing)))
                            this.handleVideoPlay(localStorage.bookmark)
