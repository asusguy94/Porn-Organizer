...
class Star
{
	...
	function freeOnes_beta($id)
	{
		...
		$baseURL = "https://www.freeones.vg";
		$base = "$baseURL/search?q=" . strtolower(str_replace(' ', '%20', $data['name']));

		$html = new simple_html_dom();

		$base_content = file_get_contents($base);
		$html->load($base_content);

		$pageCheck = $html->find('.profile-image-container');
		if (!count($pageCheck)) {
			$searchElement = $html->find('.px-3 a', 0);
			$star = "$baseURL$searchElement->href";
		} else {
			$star = "$baseURL/" . strtolower(str_replace(' ', '-', $data['name']));
		}
		$star_content = file_get_contents($star);
		$html->load($star_content);

		/* Activity */
		$year_start = $html->find('.timeline-horizontal p', 0)->plaintext;
		if (!strlen($year_start)) $year_start = '';

		$year_end = $html->find('.timeline-horizontal p', 3)->plaintext;
		if (!strlen($year_end) || strtolower($year_end) === 'now') $year_end = '';

		/* Bio */
		$bio = "$star/profile";
		$bio_content = file_get_contents($bio);
		$html->load($bio_content);

		// Ethnicity
		$ethnicity = $html->find('.profile-meta-item div', 2)->plaintext;
		if (!strlen($ethnicity)) $ethnicity = '';

		// Country
		/*$country = '';
		$query = $html->find('.profile-meta-item div .mb-1');
		foreach ($query AS $element) {
			foreach ($element->childNodes() AS $child) {
				$href_arr = explode('/babes?f%5Bcountry%5D=', $child->href);
				if (count($href_arr) > 1) {
					$country = $href_arr[1];
					break;
				}
		...
		foreach ($query AS $element) {
			foreach ($element->childNodes() AS $child) {
				$href_arr = explode('/babes?f%5BdateOfBirth%5D=', $child->href);
				if (count($href_arr) > 1) {
					$date = $href_arr[1];
					break;
				}
		...
		// Eye
		$eyecolor = $html->find('.profile-meta-item div .mb-0', 4)->plaintext;
		if (!strlen($eyecolor)) $eyecolor = '';

		// Hair
		$haircolor = $html->find('.profile-meta-item div .mb-0', 5)->plaintext;
		if (!strlen($haircolor)) $haircolor = '';

		// Height
		$height = $html->find('.profile-meta-item div .mb-0', 6)->plaintext;

		$cm = explode('cm', $height);
		if (count($cm) > 1) {
			$height = $cm[0];
		} else {
			$height = '';
		...
		// Weight
		$weight = $html->find('.profile-meta-item div .mb-0', 7)->plaintext;

		$kg = explode('kg', $weight);
		if (count($kg) > 1) {
			$weight = $kg[0];
		} else {
			$weight = '';
		...
		// Breast
		$breast_str = $html->find('.profile-meta-item div .mb-0', 8)->plaintext;
		...
		//$date_class = new Date();
		if (is_null($data['breast']) && strlen($breast)) {
			$saveProp('breast', $breast);
		}
		if (is_null($data['eyecolor']) && strlen($eyecolor)) {
			$saveProp('eyecolor', $eyecolor);
		}
		if (is_null($data['haircolor']) && strlen($haircolor)) {
			$saveProp('haircolor', $haircolor);
		}
		if (is_null($data['ethnicity']) && strlen($ethnicity)) {
			$saveProp('ethnicity', $ethnicity);
		}
		/*if (is_null($data['country']) && strlen($country)) {
			$saveProp('country', $country);*/
		...
		//}
		/*if (is_null($data['birthdate']) && strlen($date)) {
			$saveProp('birthdate', $date_class->stringToDate($date));
		}*/
		if (is_null($data['height']) && strlen($height)) {
			$saveProp('height', $height);
		}
		if (is_null($data['weight']) && strlen($weight)) {
			$saveProp('weight', $weight);
		}
		if (is_null($data['start']) && strlen($year_start)) {
			$saveProp('start', $year_start);
		}
		if (is_null($data['end']) && strlen($year_end)) {
			$saveProp('end', $year_end);
		}
		//Basic::reload();
	}
